...
bool persist_display()
{
    for (int i = 0; i < 5; i++)
    {
        if (!write_int(catalogmenu_section[i]))
            return false;
        if (!write_int(catalogmenu_rows[i]))
            return false;
        if (!write_int(catalogmenu_row[i]))
            return false;
        for (int j = 0; j < 6; j++)
            if (!write_int(catalogmenu_item[i][j]))
                return false;
    }
    for (int i = 0; i < 3; i++)
    {
        for (int j = 0; j < 6; j++)
        {
            if (!write_int(custommenu_length[i][j]))
                return false;
            if (fwrite(custommenu_label[i][j], 1, 7, gfile) != 7)
                return false;
        }
    }
    for (int i = 0; i < 9; i++)
        if (!write_arg(progmenu_arg + i))
            return false;
    for (int i = 0; i < 9; i++)
        if (!write_bool(progmenu_is_gto[i]))
            return false;
    for (int i = 0; i < 6; i++)
    {
        if (!write_int(progmenu_length[i]))
            return false;
        if (fwrite(progmenu_label[i], 1, 7, gfile) != 7)
            return false;
    }
    if (fwrite(display, 1, 272, gfile) != 272)
        return false;
    if (!write_int(appmenu_exitcallback))
        return false;
    return true;
}
...
bool unpersist_display(int version)
{
    if (state_is_portable)
    {
        for (int i = 0; i < 5; i++)
        {
            if (!read_int(&catalogmenu_section[i]))
                return false;
            if (!read_int(&catalogmenu_rows[i]))
                return false;
            if (!read_int(&catalogmenu_row[i]))
                return false;
            for (int j = 0; j < 6; j++)
                if (!read_int(&catalogmenu_item[i][j]))
                    return false;
        }
        for (int i = 0; i < 3; i++)
        {
            for (int j = 0; j < 6; j++)
            {
                if (!read_int(&custommenu_length[i][j]))
                    return false;
                if (fread(custommenu_label[i][j], 1, 7, gfile) != 7)
                    return false;
            }
        }
        for (int i = 0; i < 9; i++)
        ...
        {
            for (int i = 0; i < 9; i++)
                if (!read_bool(&progmenu_is_gto[i]))
                    return false;
        }
        for (int i = 0; i < 6; i++)
        {
            if (!read_int(&progmenu_length[i]))
                return false;
            if (fread(progmenu_label[i], 1, 7, gfile) != 7)
                return false;
        }
        if (fread(display, 1, 272, gfile) != 272)
            return false;
        if (!read_int(&appmenu_exitcallback))
            return false;
    }
    else
    {
        ...
        is_dirty = false;
        if (fread(catalogmenu_section, 1, 5 * sizeof(int), gfile) != 5 * sizeof(int))
            return false;
        if (fread(catalogmenu_rows, 1, 5 * sizeof(int), gfile) != 5 * sizeof(int))
            return false;
        if (fread(catalogmenu_row, 1, 5 * sizeof(int), gfile) != 5 * sizeof(int))
            return false;
        if (fread(catalogmenu_item, 1, 30 * sizeof(int), gfile) != 30 * sizeof(int))
            return false;
        ...
        if (version < 7)
        {
             ...
             */
            if (fread(custommenu_cmd, 1, 18 * sizeof(int), gfile) != 18 * sizeof(int))
                return false;
        }
        if (fread(custommenu_length, 1, 18 * sizeof(int), gfile) != 18 * sizeof(int))
            return false;
        if (fread(custommenu_label, 1, 126, gfile) != 126)
            return false;
        if (version < 7)
        {
        ...
        if (fread(progmenu_length, 1, 6 * sizeof(int), gfile) != 6 * sizeof(int))
            return false;
        if (fread(progmenu_label, 1, 42, gfile) != 42)
            return false;
        if (fread(display, 1, 272, gfile) != 272)
            return false;
        if (fread(&appmenu_exitcallback, 1, sizeof(int), gfile) != sizeof(int))
            return false;
    }
    return true;
}
                 ...
                 bool highlight_final_end = true)
{
    ...
    else if (highlight_final_end && cmd == CMD_END && current_prgm == prgms_count - 1)
    {
...
void display_error(int error, bool print)
{
    ...
    if (print && (flags.f.trace_print || flags.f.normal_print) && flags.f.printer_exists)
...
void display_command(int row)
{
    ...
    int catsect;
    int hide = pending_command == CMD_VMEXEC || pending_command == CMD_PMEXEC || (pending_command == CMD_XEQ && xeq_invisible && get_front_menu() == MENU_CATALOG && ((catsect = get_cat_section()) == CATSECT_PGM || catsect == CATSECT_PGM_ONLY));
    ...
    if (pending_command_arg.type == ARGTYPE_IND_NUM || pending_command_arg.type == ARGTYPE_IND_STK || pending_command_arg.type == ARGTYPE_IND_STR)
        string2buf(buf, 22, &bufptr, "IND ", 4);

    if (pending_command_arg.type == ARGTYPE_NUM || pending_command_arg.type == ARGTYPE_IND_NUM)
    {
        int d = 1, i;
        int leadingzero = 1;
    ...
    else if (pending_command_arg.type == ARGTYPE_STK || pending_command_arg.type == ARGTYPE_IND_STK)
    {
        string2buf(buf, 22, &bufptr, "ST ", 3);
        char2buf(buf, 22, &bufptr, pending_command_arg.val.stk);
    }
    else if (pending_command_arg.type == ARGTYPE_STR || pending_command_arg.type == ARGTYPE_IND_STR)
    {
        char2buf(buf, 22, &bufptr, '"');
...
static int set_appmenu(int menuid, bool exitall)
{
    if (mode_appmenu != MENU_NONE && appmenu_exitcallback != 0)
    {
        ...
        {
        case 1:
            return appmenu_exitcallback_1(menuid, exitall);
        case 2:
            return appmenu_exitcallback_2(menuid, exitall);
        case 3:
            return appmenu_exitcallback_3(menuid, exitall);
        case 4:
            return appmenu_exitcallback_4(menuid, exitall);
        case 5:
            return appmenu_exitcallback_5(menuid, exitall);
        default:
...
static void draw_catalog()
{
    ...
    else if (catsect == CATSECT_PGM || catsect == CATSECT_PGM_ONLY || catsect == CATSECT_PGM_SOLVE || catsect == CATSECT_PGM_INTEG || catsect == CATSECT_PGM_MENU)
    {
        ...
        for (i = labels_count - 1; i >= 0; i--)
        {
            ...
            if (catsect == CATSECT_PGM || catsect == CATSECT_PGM_ONLY)
            {
                show_this_label = labels[i].length > 0 || i == 0 || labels[i - 1].prgm != labels[i].prgm;
            }
            else
            {
    ...
    else
    {
        ...
        case CATSECT_REAL_ONLY:
            show_cpx = show_mat = show_list = 0;
            break;
        case CATSECT_CPX:
            show_real = show_mat = show_list = 0;
            break;
        case CATSECT_MAT:
        case CATSECT_MAT_ONLY:
            show_real = show_cpx = show_list = 0;
            break;
        }
...
void redisplay()
{
    ...
    else if (menu_id == MENU_CUSTOM1 || menu_id == MENU_CUSTOM2 || menu_id == MENU_CUSTOM3)
    {
        int r = menu_id - MENU_CUSTOM1;
        if (flags.f.local_label && !(mode_command_entry && incomplete_argtype == ARG_CKEY))
        {
            for (i = 0; i < 5; i++)
            {
                char c = (r == 0 ? 'A' : 'F') + i;
        ...
        for (i = 0; i < 6; i++)
        {
            ...
            else
            {
                ...
                if (is_flag)
                {
                    ...
                    {
                    case CMD_FIX:
                        is_flag = flags.f.fix_or_all && !flags.f.eng_or_all;
                        break;
                    case CMD_SCI:
                        is_flag = !flags.f.fix_or_all && !flags.f.eng_or_all;
                        break;
                    ...
                    case CMD_POFF:
                        ...
                        break;
                    case CMD_MAN:
                        is_flag = !flags.f.trace_print && !flags.f.normal_print;
                        break;
                    case CMD_NORM:
                        is_flag = !flags.f.trace_print && flags.f.normal_print;
                        break;
        ...
        if (dat->trace)
        {
            if (dat->cmd == CMD_LBL || dat->first)
            {
                ...
                dat->len = 1 + prgmline2buf(dat->buf + 1, 100 - 1, dat->line,
                                            ...
                                            &dat->arg, orig_num);
                if (dat->cmd == CMD_LBL || dat->cmd == CMD_END || dat->lines == 1)
                {
                    print_lines(dat->buf, dat->len, 1);
                    printed = 1;
                    dat->len = 0;
...
int command2buf(char *buf, int len, int cmd, const arg_struct *arg)
{
    ...
    else if (cmdspec->argtype != ARG_NONE)
    {
        if (cmdspec->name_length > 0)
            char2buf(buf, len, &bufptr, ' ');
        if (arg->type == ARGTYPE_IND_NUM || arg->type == ARGTYPE_IND_STK || arg->type == ARGTYPE_IND_STR)
            string2buf(buf, len, &bufptr, "IND ", 4);
        if (arg->type == ARGTYPE_NUM || arg->type == ARGTYPE_IND_NUM)
        {
        ...
        else if (arg->type == ARGTYPE_STK || arg->type == ARGTYPE_IND_STK)
        {
...
lbl_03:
    ...
    if (newmenu != MENU_NONE)
    {
        ...
        else if (newmenu == MENU_PROGRAMMABLE)
        {
             ...
             */
            mode_updown = progmenu_arg[6].type != ARGTYPE_NONE || progmenu_arg[7].type != ARGTYPE_NONE;
        }
        else
        {
...
void set_plainmenu(int menuid)
{
    ...
    else if (menuid == MENU_CUSTOM1 || menuid == MENU_CUSTOM2 || menuid == MENU_CUSTOM3)
    {
        mode_plainmenu = menuid;
        mode_plainmenu_sticky = 1;
        redisplay();
    ...
    else
    {
        ...
        mode_updown = mode_plainmenu == MENU_CATALOG || mode_plainmenu != MENU_NONE && menus[mode_plainmenu].next != MENU_NONE;
        shell_annunciators(mode_updown, -1, -1, -1, -1, -1);
    }
}
