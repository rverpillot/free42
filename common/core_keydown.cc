...
{
    if (get_front_menu() == MENU_BASE_A_THRU_F && !no_menu_key_this_time && (key == KEY_SIGMA || key == KEY_INV || key == KEY_SQRT || key == KEY_LOG || key == KEY_LN || key == KEY_XEQ))
        return 1;
    return !shift && (key == KEY_0 || key == KEY_1 || key == KEY_2 || key == KEY_3 || key == KEY_4 || key == KEY_5 || key == KEY_6 || key == KEY_7 || key == KEY_8 || key == KEY_9 || key == KEY_DOT || key == KEY_E);
}
...
void keydown(int shift, int key)
{
    ...
    if (mode_number_entry && get_base() == 16 && key == KEY_SIGMA && get_front_menu() == MENU_BASE)
    {
    ...
    if (mode_number_entry && !is_number_key(shift, key) && (key != KEY_CHS || shift || basekeys() || get_base() != 10) && (key != KEY_BSP || shift))
    {
        ...
        else if ((flags.f.trace_print || flags.f.normal_print) && flags.f.printer_exists)
            deferred_print = 1;
    ...
    if (mode_command_entry && (shift || get_front_menu() == MENU_NONE) && (key == KEY_UP || key == KEY_DOWN))
    {
    ...
    if (flags.f.prgm_mode && (key == KEY_UP || key == KEY_DOWN) && (shift || get_front_menu() == MENU_NONE))
    {
    ...
    if (!flags.f.prgm_mode && key == KEY_UP && (shift || get_front_menu() == MENU_NONE))
    {
        /* BST in normal or alpha mode */
        if (mode_alpha_entry && (flags.f.trace_print || flags.f.normal_print) && flags.f.printer_exists)
            docmd_pra(NULL);
        mode_alpha_entry = false;
        clear_all_rtns();
        bst();
...
void keydown_number_entry(int shift, int key)
{
    ...
    else if (key == KEY_CHS)
    {
        ...
        if (exp_pos != -1)
        {
            ...
            phloat d;
        undo_chs_exp:
            if (cmdline_length > exp_pos + 1 && cmdline[exp_pos + 1] == '-')
            {
...
void keydown_command_entry(int shift, int key)
{
    if (mode_commandmenu == MENU_ST || mode_commandmenu == MENU_IND_ST)
    {
        ...
        if (!shift && ((menukey >= 0 && menukey <= 4) || (menukey == 5 && mode_commandmenu == MENU_IND_ST)))
        {
    ...
    if (incomplete_command == CMD_LBL && incomplete_length == 0 && mode_commandmenu != MENU_CATALOG)
    {
         ...
         */
        if ((mode_commandmenu == MENU_ALPHA1 || mode_commandmenu == MENU_ALPHA2) && key >= 1024 + '0' && key <= 1024 + '9')
            switch (key - 1024)
            {
            case '0':
                key = KEY_0;
        ...
        if (key == KEY_EXIT && mode_commandmenu >= MENU_ALPHA_ABCDE1 && mode_commandmenu <= MENU_ALPHA_MISC2)
        {
    ...
    if (incomplete_command == CMD_LBL && !incomplete_alpha && incomplete_length == 1 && shift && key == KEY_ENTER)
    {
    ...
    if ((incomplete_command == CMD_ASTO || incomplete_command == CMD_ARCL) && incomplete_length == 1 && mode_commandmenu == MENU_NONE && mode_alphamenu >= MENU_ALPHA1 && mode_alphamenu <= MENU_ALPHA_MISC2)
    {
    ...
    if (incomplete_argtype == ARG_CKEY)
    {
        ...
        if (menukey != -1)
        {
            pending_command = CMD_ASGN01 + menukey + 6 * (mode_commandmenu - MENU_CUSTOM1);
            finish_command_entry(true);
        }
    ...
    if (mode_commandmenu == MENU_CATALOG)
    {
        ...
        if (menukey != -1)
        {
            ...
            else
            {
                ...
                if (catsect == CATSECT_PGM || catsect == CATSECT_PGM_ONLY)
                {
                    ...
                    {
                        /* END or .END. */
                        if (incomplete_command != CMD_GTODOT && incomplete_command != CMD_PRP && (flags.f.prgm_mode || (incomplete_command != CMD_GTO && incomplete_command != CMD_XEQ && incomplete_command != CMD_CLP)))
                        {
                            squeak();
                            return;
                        }
                    ...
                    pending_command = incomplete_command;
                    if (incomplete_command == CMD_GTO || incomplete_command == CMD_GTODOT || incomplete_command == CMD_XEQ || incomplete_command == CMD_CLP || incomplete_command == CMD_PRP)
                    {
                ...
                pending_command_arg.type =
                    incomplete_ind ? ARGTYPE_IND_STR : ARGTYPE_STR;
                if (catsect == CATSECT_FCN || catsect >= CATSECT_EXT_TIME && catsect <= CATSECT_EXT_MISC)
                {
        ...
        if (key == KEY_EXIT)
        {
            if (catsect == CATSECT_FCN || catsect == CATSECT_PGM || catsect == CATSECT_REAL || catsect == CATSECT_CPX || catsect == CATSECT_MAT)
            {
                set_cat_section(CATSECT_TOP);
                redisplay();
            }
            else if (catsect == CATSECT_EXT_TIME || catsect == CATSECT_EXT_XFCN || catsect == CATSECT_EXT_BASE || catsect == CATSECT_EXT_PRGM || catsect == CATSECT_EXT_STK || catsect == CATSECT_EXT_MISC)
            {
                set_cat_section(CATSECT_EXT);
                redisplay();
            }
    ...
    if (!incomplete_alpha)
    {
        ...
        if (incomplete_command == CMD_STO && !shift && incomplete_length == 0 && (key == KEY_DIV || key == KEY_MUL || key == KEY_SUB || key == KEY_ADD))
        {
            switch (key)
            {
            case KEY_DIV:
        ...
        if (incomplete_command == CMD_RCL && !shift && incomplete_length == 0 && (key == KEY_DIV || key == KEY_MUL || key == KEY_SUB || key == KEY_ADD))
        {
            switch (key)
            {
            case KEY_DIV:
                incomplete_command = CMD_RCL_DIV;
                break;
            case KEY_MUL:
                incomplete_command = CMD_RCL_MUL;
                break;
            case KEY_SUB:
                incomplete_command = CMD_RCL_SUB;
                break;
            case KEY_ADD:
                incomplete_command = CMD_RCL_ADD;
                break;
            }
            redisplay();
            return;
        }
        if (incomplete_command == CMD_GTO && !incomplete_ind && !shift && incomplete_length == 0 && key == KEY_DOT)
        {
        ...
        if (incomplete_command == CMD_GTODOT && !shift && incomplete_length == 0 && key == KEY_DOT)
        {
        ...
        if (key == KEY_ENTER)
        {
            ...
            {
                if (incomplete_ind && !flags.f.prgm_mode && !vars_exist(CATSECT_REAL))
                    squeak();
                else if (incomplete_ind || incomplete_argtype == ARG_VAR || incomplete_argtype == ARG_REAL || incomplete_argtype == ARG_NAMED || incomplete_argtype == ARG_LBL || incomplete_argtype == ARG_PRGM)
                {
                    incomplete_alpha = true;
                    set_menu(MENULEVEL_COMMAND, MENU_ALPHA1);
                    redisplay();
        ...
        if (incomplete_length == 0 && !shift && key == KEY_DOT)
        {
            if (incomplete_ind)
            {
                if (incomplete_argtype == ARG_VAR || incomplete_argtype == ARG_REAL || incomplete_argtype == ARG_MAT || incomplete_argtype == ARG_RVAR || incomplete_argtype == ARG_NAMED || incomplete_argtype == ARG_LBL || incomplete_argtype == ARG_PRGM || incomplete_argtype == ARG_NUM9 || incomplete_argtype == ARG_NUM11 || incomplete_argtype == ARG_NUM99)
                {
            ...
            {
                if (incomplete_argtype == ARG_VAR || incomplete_argtype == ARG_REAL)
                {
                    set_menu(MENULEVEL_COMMAND, MENU_IND_ST);
                    redisplay();
                }
                else if (incomplete_argtype == ARG_NUM9 || incomplete_argtype == ARG_NUM11 || incomplete_argtype == ARG_NUM99 || incomplete_argtype == ARG_LBL)
                {
                    incomplete_ind = true;
                    incomplete_maxdigits = 2;
                    set_catalog_menu(CATSECT_REAL_ONLY);
        ...
        if (incomplete_length < incomplete_maxdigits && !shift && key == KEY_BSP)
        {
            if (incomplete_length == 0)
            {
                pending_command = CMD_NULL;
            ...
            else
            {
                ...
                if (incomplete_length == 0)
                {
                    if (incomplete_command >= CMD_KEY1G && incomplete_command <= CMD_KEY9X)
    ...
    do_incomplete_alpha:
        ...
        else if (mode_commandmenu != MENU_NONE && (menukey = find_menu_key(key)) != -1)
        {
    ...
    nocharkey1:
        ...
        if (key == KEY_EXIT)
        {
            ...
            {
                int catsect = get_cat_section();
                if (catsect == CATSECT_PGM || catsect == CATSECT_REAL || catsect == CATSECT_CPX || catsect == CATSECT_MAT)
                {
                    set_cat_section(CATSECT_TOP);
                    redisplay();
                    return;
        ...
        if (!shift && key == KEY_BSP)
        {
            if (incomplete_command >= CMD_KEY1G && incomplete_command <= CMD_KEY9X)
            {
                if (incomplete_length == 0)
                {
                    if (mode_commandmenu >= MENU_ALPHA1 && mode_commandmenu <= MENU_ALPHA_MISC2)
                        start_incomplete_command(
            ...
            if (incomplete_length == 0)
            {
                ...
                else if (mode_commandmenu == MENU_CATALOG && ((catsect = get_cat_section()) == CATSECT_FCN || catsect == CATSECT_PGM || catsect == CATSECT_REAL || catsect == CATSECT_CPX || catsect == CATSECT_MAT))
                {
                    set_catalog_menu(CATSECT_TOP);
                    redisplay();
                }
                else if (mode_commandmenu == MENU_CATALOG && ((catsect = get_cat_section()) == CATSECT_EXT_TIME || catsect == CATSECT_EXT_XFCN || catsect == CATSECT_EXT_BASE || catsect == CATSECT_EXT_PRGM || catsect == CATSECT_EXT_STK || catsect == CATSECT_EXT_MISC))
                {
            ...
            if (incomplete_length == 0)
            {
                ...
                {
                out_of_alpha:
                    if (incomplete_ind || incomplete_argtype != ARG_RVAR)
                        incomplete_alpha = false;
                    if (incomplete_ind)
                        set_catalog_menu(CATSECT_REAL_ONLY);
                    else if (incomplete_argtype == ARG_VAR)
        ...
        if (key == KEY_ENTER)
        {
            ...
            {
                int catsect;
                if (mode_commandmenu == MENU_NONE || (mode_commandmenu == MENU_CATALOG && (catsect = get_cat_section()) != CATSECT_FCN && catsect != CATSECT_PGM && catsect != CATSECT_REAL && catsect != CATSECT_CPX && catsect != CATSECT_MAT && catsect != CATSECT_EXT_TIME && catsect != CATSECT_EXT_XFCN && catsect != CATSECT_EXT_BASE && catsect != CATSECT_EXT_PRGM && catsect != CATSECT_EXT_STK && catsect != CATSECT_EXT_MISC))
                {
                    set_menu(MENULEVEL_COMMAND, MENU_ALPHA1);
                    redisplay();
                    return;
                }
                else if ((incomplete_command != CMD_ASSIGNa && incomplete_command != CMD_CLP && incomplete_command != CMD_PRP) || mode_commandmenu < MENU_ALPHA1 || mode_commandmenu > MENU_ALPHA_MISC2)
                {
                    squeak();
                    return;
                }
            ...
            if (!incomplete_ind && (incomplete_command == CMD_GTO || incomplete_command == CMD_XEQ || incomplete_command == CMD_LBL || (incomplete_command >= CMD_KEY1G && incomplete_command <= CMD_KEY9X)) && incomplete_length == 1 && ((incomplete_str[0] >= 'A' && incomplete_str[0] <= 'J') || (incomplete_str[0] >= 'a' && incomplete_str[0] <= 'e')))
            {
...
nocharkey2:
    ...
    if (!shift && key == KEY_BSP)
    {
        if (flags.f.prgm_mode)
        {
            ...
            else
            {
                ...
                if (line != 0 && (current_prgm != prgms_count - 1 || prgms[current_prgm].text[pc] != CMD_END))
                {
                    delete_command(pc);
                    pc = line2pc(line - 1);
                }
                prgm_highlight_row = 0;
                if (mode_alphamenu != MENU_ALPHA1 && mode_alphamenu != MENU_ALPHA2)
    ...
    if (!shift)
    {
        ...
        case KEY_RUN:
            ...
            break;
        default:
            command = key >= 2048 ? key - 2048 : CMD_NONE;
            break;
        }
    }
    else
    {
        ...
        {
        case KEY_CHS:
            set_plainmenu(MENU_MODES1);
            break;
        case KEY_E:
            set_plainmenu(MENU_DISP);
            break;
        case KEY_BSP:
            set_plainmenu(MENU_CLEAR1);
            break;
        case KEY_7:
            set_solve_integ(1);
            break;
        ...
        case KEY_4:
            ...
            break;
        case KEY_5:
            set_plainmenu(MENU_CONVERT1);
            break;
        case KEY_6:
            set_plainmenu(MENU_FLAGS);
            break;
        case KEY_MUL:
            set_plainmenu(MENU_PROB);
            break;
        case KEY_2:
            set_plainmenu(MENU_CUSTOM1);
            break;
        case KEY_3:
            set_plainmenu(MENU_PGM_FCN1);
            break;
        case KEY_SUB:
            set_plainmenu(MENU_PRINT1);
            break;
        case KEY_0:
            set_plainmenu(MENU_TOP_FCN);
            break;
        case KEY_DOT:
            show();
            pending_command = CMD_LINGER1;
...
void keydown_normal_mode(int shift, int key)
{
    ...
    if (flags.f.prgm_mode && !shift && key == KEY_BSP)
    {
        ...
        if (line == 0)
            return;
        if (current_prgm != prgms_count - 1 || prgms[current_prgm].text[pc] != CMD_END)
            delete_command(pc);
        pc = line2pc(line - 1);
        prgm_highlight_row = 0;
        redisplay();
    ...
    if (!no_menu_key_this_time && (mode_appmenu != MENU_NONE || mode_plainmenu != MENU_NONE || mode_transientmenu != MENU_NONE) && mode_alphamenu == MENU_NONE && mode_commandmenu == MENU_NONE)
    {
        int menukey = find_menu_key(key);
        int menu, level;
        ...
        if (menukey != -1)
        {
            if (menu == MENU_CUSTOM1 || menu == MENU_CUSTOM2 || menu == MENU_CUSTOM3)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            else if ((cmd == CMD_SST || cmd == CMD_SST_UP || cmd == CMD_SST_RT) && flags.f.prgm_mode)
                            {
                                sst();
                                pending_command = CMD_NONE;
                                redisplay();
            ...
            else if (menu == MENU_CATALOG)
            {
                ...
                else if (catsect == CATSECT_PGM || catsect == CATSECT_PGM_ONLY)
                {
                    ...
                    if (flags.f.prgm_mode && labels[labelindex].length == 0)
                    {
                    ...
                    xeq_invisible = 1;
                    if (!flags.f.prgm_mode && (level == MENULEVEL_TRANSIENT || !mode_plainmenu_sticky))
                    {
                ...
                else if (catsect == CATSECT_PGM_SOLVE || catsect == CATSECT_PGM_INTEG || catsect == CATSECT_PGM_MENU)
                {
                    int labelindex = get_cat_item(menukey);
                    int i;
                    if (labelindex == -1)
                ...
                else if (catsect == CATSECT_FCN || catsect >= CATSECT_EXT_TIME && catsect <= CATSECT_EXT_MISC)
                {
                    ...
                    if (cmd == -1)
                        if (flags.f.prgm_mode)
                        {
                            ...
                            cmd = CMD_NULL;
                    if (level == MENULEVEL_TRANSIENT || !mode_plainmenu_sticky)
            ...
            send_it_off:
                    ...
                    (cmd_array[pending_command].flags & FLAG_IMMED) == 0)
                {
                    ...
                    prgm_highlight_row = 1;
                    pending_command = CMD_NONE;
                    if (level == MENULEVEL_TRANSIENT || (level == MENULEVEL_PLAIN && !mode_plainmenu_sticky))
                        set_menu(level, MENU_NONE);
                    redisplay();
                }
                return;
        ...
        if (!shift && (key == KEY_UP || key == KEY_DOWN))
        {
            ...
            else if (flags.f.local_label && (menu == MENU_CUSTOM1 || menu == MENU_CUSTOM2 || menu == MENU_CUSTOM3))
            {
            ...
            else if (!core_settings.allow_big_stack && (menu == MENU_MODES1 && key == KEY_UP || menu == MENU_MODES4 && key == KEY_DOWN))
            {
        ...
        if (key == KEY_EXIT)
        {
            if (menu == MENU_CATALOG)
            {
                ...
                if (catsect == CATSECT_FCN || catsect == CATSECT_PGM || catsect == CATSECT_REAL || catsect == CATSECT_CPX || catsect == CATSECT_MAT)
                    set_cat_section(CATSECT_TOP);
                else if (catsect >= CATSECT_EXT_TIME && catsect <= CATSECT_EXT_MISC)
                    set_cat_section(CATSECT_EXT);
                else
                    set_menu(level, MENU_NONE);
            }
    ...
    if (!shift)
    {
        ...
        case KEY_TAN:
            ...
            break;
        case KEY_ENTER:
            command = flags.f.big_stack ? CMD_DUP : CMD_ENTER;
            break;
        case KEY_SWAP:
            command = CMD_SWAP;
            break;
        case KEY_CHS:
            command = basekeys() ? CMD_BASECHS : CMD_CHS;
            break;
        case KEY_BSP:
            command = flags.f.big_stack ? CMD_DROP : CMD_CLX;
            break;
        case KEY_DIV:
            command = basekeys() ? CMD_BASEDIV : CMD_DIV;
            break;
        case KEY_DOWN:
            command = CMD_SST;
            break;
        case KEY_MUL:
            command = basekeys() ? CMD_BASEMUL : CMD_MUL;
            break;
        case KEY_SUB:
            command = basekeys() ? CMD_BASESUB : CMD_SUB;
            break;
        case KEY_EXIT:
            input_length = 0;
            pending_command = CMD_CANCELLED;
