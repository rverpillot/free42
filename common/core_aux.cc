...
int find_vvar(const char *name, int namelength)
{
  ...
  for (; ix < VVAR_CNT; ix++)
  {
    if (vvars[ix].namelen != namelength)
      continue;
...
*/
  ...
  int get_pgm_line(pgm_line_t *p, int line)
  {
    ...
    while (!p->is_end && p->line < line)
    {
      if (p->line >= 0)
      {
        get_next_command(&p->pc, &cmd, &pgm_arg, 0, NULL);
        p->is_end = cmd == CMD_END;
      }
      p->line++;
      len = prgmline2buf(p->buf, p->buflen, p->line, false, cmd, &pgm_arg, NULL, false, true);
    }
    return len;
  }
